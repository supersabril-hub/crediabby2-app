...
# 5. FORMULARIO CON LÓGICA DE LIMPIEZA CORREGIDA
# Usamos un formulario oficial de Streamlit para agrupar los datos
with st.form("form_credabby", clear_on_submit=True):
    st.subheader("📝 Nuevo Reporte de Pago")
    
    col_a, col_b = st.columns(2)
    with col_a:
        # El cobrador se escribe aparte para que persista fuera del clear_on_submit si fuera necesario
        # Pero aquí lo mantendremos simple:
...
if boton_enviar:
    # Guardamos el nombre del cobrador en la sesión para la próxima vez
    ...
    if len(referencia) != 6 or not referencia.isdigit():
        ...
        if ref_buscada in df['Referencia'].astype(str).values:
            ...
            # Al usar clear_on_submit=True en el form, los campos se limpian solos
            # Solo forzamos un reinicio para asegurar que el nombre del cobrador se mantenga
            st.rerun()
