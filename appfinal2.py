...
if boton_enviar:
    ...
    # Si NO es efectivo, validamos que tenga 6 dígitos obligatorios
    elif not es_efectivo and (len(referencia) != 6 or not referencia.isdigit()):
        st.error("❌ ERROR: Para este método de pago la referencia debe tener 6 números.")
    else:
        # Si es efectivo y está vacío, el sistema le asigna un guion automáticamente
        ref_final = referencia if referencia else "-"
        
        # Solo verificamos duplicados si hay una referencia numérica
        duplicado = False
        if ref_final != "-":
        ...
        if duplicado:
            ...
            st.error(f"❌ ¡PAGO DUPLICADO! Ya reportado por {pago_existente['Cobrador']} el {pago_existente['Fecha Reporte']}")
        else:
