...
# 2. GESTIÓN DE SESIÓN (Para recordar al cobrador seleccionado)
...
with st.form("form_credabby", clear_on_submit=True):
    ...
    with col_a:
        # LISTA DE COBRADORES INTEGRADA
        lista_cobradores = ["Seleccione su nombre", "Anthony", "Gabriela", "Emely", "Ninoska"]
        
        # Si ya seleccionó uno antes, intentamos pre-seleccionarlo
        indice_previo = 0
...
if boton_enviar:
    ...
    if empleado == "Seleccione su nombre":
        st.warning("⚠️ Por favor, selecciona tu nombre de la lista.")
    elif len(referencia) != 6 or not referencia.isdigit():
