...
# 5. FORMULARIO CON LIMPIEZA AUTOMÁTICA
...
# 6. LÓGICA DE PROCESAMIENTO Y MENSAJE DE DUPLICADO DETALLADO
if boton_enviar:
    ...
    if len(referencia) != 6 or not referencia.isdigit():
        ...
        if ref_buscada in df['Referencia'].astype(str).values:
            # Extraemos la fila completa del pago que ya existía
            pago_existente = df[df['Referencia'].astype(str) == ref_buscada].iloc[-1]
            
            st.error(f"""
                ### ❌ ¡PAGO DUPLICADO DETECTADO!
                Esta referencia ya fue ingresada con los siguientes detalles:
                * **Reportado por:** {pago_existente['Cobrador']}
                * **Fecha del reporte:** {pago_existente['Fecha Reporte']}
                * **Cliente indicado:** {pago_existente['Cliente']}
                * **Monto:** {pago_existente['Monto']}
                
                **Verifica con tu supervisor si se trata del mismo pago.**
            """)
        else:
            # REGISTRO EXITOSO
            nueva_entrada = {
...
# 7. PANEL DE CONTROL
st.markdown("---")
with st.expander("📊 Ver Historial de Pagos (Revisión Administrativa)"):
    if not df.empty:
        ...
        st.download_button(label="📥 Descargar Reporte para Excel", data=csv_data, file_name='reporte_crediabby.csv', mime='text/csv')
    else:
